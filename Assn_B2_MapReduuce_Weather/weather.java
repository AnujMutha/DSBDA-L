...
import org.apache.hadoop.util.ToolRunner;
  ...
  This is an Hadoop MapReduce application for Working on weather data It reads
  ...
  To run binhadoop jar targetweather-1.0.jar [-m imapsi] [-r
  ireducesi] iin-dir for job 1i iout-dir for job 1i iout-dir
  for job 2i
...
public class Weather extends Configured implements Tool {
	...
	public static class MapClass extends MapReduceBase
		...
		public void map(LongWritable key, Text value,
			...
			boolean skip = false;
			loopwhile (itr.hasMoreTokens() && counter13) {
				...
				case 2
					...
					}else if(hour5 && hour=11){
						str = str.concat(_section1);
					}else if(hour11 && hour=17){
						str = str.concat(_section2);
					}else if(hour17 && hour=23){
						str = str.concat(_section3);
					}
					
					if(hour4 && hour=10){
						str = str.concat(_section1);
					}else if(hour10 && hour=16){
						str = str.concat(_section2);
					}else if(hour16 && hour=22){
						str = str.concat(_section3);
					}else{
						str = str.concat(_section4);
					}
	  ...
	  ( bstation_yearmonthb, btemperature,dew point , wind speedb).
	...
	public static class Reduce extends MapReduceBase
			implements ReducerText, Text, Text, Text {
		...
		public void reduce(Text key, IteratorText values,
			...
			System.out.println(key.toString()+ count is +count+ sum of temp is +sum_temp+ sum of dew is +sum_dew+ sum of wind is +sum_wind+n);
	...
	static int printUsage() {
	  ...
	  The main driver for weather mapreduce program.
